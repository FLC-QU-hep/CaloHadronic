...
def sort(samples, cond_E):
    ...
    return hits_per_layer_all_tot, hits_per_layer_all_tot2, idx_hits_sorted
...
def generic_rescales(fs, padding_mask, Ymax, Ymin, max_clip, config):
...
def light_gen_showers_batch(
    ...
    Xmax, Xmin, Zmax, Zmin, global_Ymin, global_Ymax = 450, -450, 450, -450, 0, 78
    ...
    for evt_id in range(0, num, bs):
        ...
        fs = generic_rescales(fs, padding_mask, Ymax, Ymin, max_clip, config)

        if config.only_ecal:
            for i in range(4):
                fs[:, :, i][padding_mask] = -10  # for SF calibration
            hits_per_layer_all = samples[evt_id : evt_id + bs][
                :, :30
            ]  # [:, :Ymax]
        if config.only_hcal:
            ...
            hits_per_layer_all = samples[
                evt_id : evt_id + bs
            ]  # [:, :Ymax]

        cell_thickness = 1
        layer_bottom_pos = np.linspace(
            global_Ymin, global_Ymax - 1, global_Ymax - global_Ymin
        )
